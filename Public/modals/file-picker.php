...
if (!function_exists('tc_admin_content_file_library_html')) {
...
$filePickerUpload = static function (string $type): string {
    ...
    <section class="file-picker-screen" data-file-picker-screen="<?= e($type) ?>">
        <form class="file-picker-upload" action="/admin/content?api=file-upload&view=html&type=<?= e($type) ?>" method="post" enctype="multipart/form-data" data-ajax-form data-ajax-target="#<?= e($libraryId) ?>" data-reset="true" data-file-upload-form>
            <?= csrf_field() ?>
            <label class="dropzone file-picker-dropzone" data-dropzone>
                <input class="sr-only" type="file" name="file" accept="<?= e(tc_admin_content_file_accept($type)) ?>" required>
        ...
        <div class="file-picker-library" id="<?= e($libraryId) ?>" data-file-library>
            <?= tc_admin_content_file_library_html($type) ?>
        </div>
    </section>
    <?php
...
$body = '<div class="file-picker" data-file-picker>'
