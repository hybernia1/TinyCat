...
$artifact = trim((string) ($options['artifact'] ?? ($root . '/dist/release-2.0.29')));
...
try {
    ...
    $assert($packageName === 'tinycat-2.0.29.zip' && is_file($packagePath), 'The 2.0.29 package is present.');
    $assert(($manifest['version'] ?? null) === '2.0.29', 'Manifest targets 2.0.29.');
    ...
    $assert(str_contains($core, "public const string VERSION = '2.0.29';"), 'Extracted runtime reports 2.0.29.');
...
echo "PASS extracted 2.0.29 artifact preflight ({$checks} checks, signed inventory, routes and fresh MySQL install)\n";
