...
$artifact = trim((string) ($options['artifact'] ?? ($root . '/dist/release-2.0.27')));
...
try {
    ...
    $assert($packageName === 'tinycat-2.0.27.zip' && is_file($packagePath), 'The 2.0.27 package is present.');
    $assert(($manifest['version'] ?? null) === '2.0.27', 'Manifest targets 2.0.27.');
    ...
    $assert(str_contains($core, "public const string VERSION = '2.0.27';"), 'Extracted runtime reports 2.0.27.');
...
echo "PASS extracted 2.0.27 artifact preflight ({$checks} checks, signed inventory, routes and fresh MySQL install)\n";
