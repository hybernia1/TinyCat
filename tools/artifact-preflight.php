...
$artifact = trim((string) ($options['artifact'] ?? ($root . '/dist/release-2.0.35')));
...
try {
    ...
    $assert($packageName === 'tinycat-2.0.35.zip' && is_file($packagePath), 'The 2.0.35 package is present.');
    $assert(($manifest['version'] ?? null) === '2.0.35', 'Manifest targets 2.0.35.');
    ...
    $assert(str_contains($core, "public const string VERSION = '2.0.35';"), 'Extracted runtime reports 2.0.35.');
...
echo "PASS extracted 2.0.35 artifact preflight ({$checks} checks, signed inventory, routes and fresh MySQL install)\n";
