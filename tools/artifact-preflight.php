...
$artifact = trim((string) ($options['artifact'] ?? ($root . '/dist/release-2.0.50')));
...
try {
    ...
    $assert($packageName === 'tinycat-2.0.50.zip' && is_file($packagePath), 'The 2.0.50 package is present.');
    $assert(($manifest['version'] ?? null) === '2.0.50', 'Manifest targets 2.0.50.');
    ...
    $assert(in_array('migrations/20260813_001_remove_relation_created_at.php', (array) ($manifest['migrations'] ?? []), true), 'Patch release removes redundant relation timestamps.');
    ...
    $assert(str_contains($core, "public const string VERSION = '2.0.50';"), 'Extracted runtime reports 2.0.50.');
...
echo "PASS extracted 2.0.50 artifact preflight ({$checks} checks, signed inventory, routes and fresh MySQL install)\n";
