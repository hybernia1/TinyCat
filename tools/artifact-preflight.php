...
$artifact = trim((string) ($options['artifact'] ?? ($root . '/dist/release-2.0.26')));
...
try {
    ...
    $assert($packageName === 'tinycat-2.0.26.zip' && is_file($packagePath), 'The 2.0.26 package is present.');
    $assert(($manifest['version'] ?? null) === '2.0.26', 'Manifest targets 2.0.26.');
    ...
    $assert(($manifest['delete'] ?? null) === [], 'Patch release declares no legacy deletions.');
    ...
    $assert(str_contains($core, "public const string VERSION = '2.0.26';"), 'Extracted runtime reports 2.0.26.');
...
echo "PASS extracted 2.0.26 artifact preflight ({$checks} checks, signed inventory, routes and fresh MySQL install)\n";
