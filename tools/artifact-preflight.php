...
$artifact = trim((string) ($options['artifact'] ?? ($root . '/dist/release-2.0.38')));
...
try {
    ...
    $assert($packageName === 'tinycat-2.0.38.zip' && is_file($packagePath), 'The 2.0.38 package is present.');
    $assert(($manifest['version'] ?? null) === '2.0.38', 'Manifest targets 2.0.38.');
    ...
    $assert(str_contains($core, "public const string VERSION = '2.0.38';"), 'Extracted runtime reports 2.0.38.');
...
echo "PASS extracted 2.0.38 artifact preflight ({$checks} checks, signed inventory, routes and fresh MySQL install)\n";
