...
$artifactRoot = trim((string) ($options['artifact'] ?? ($root . '/dist/release-2.0.34')));
...
PHP;
    ...
    $newBootRunner = "define('TINYCAT', true); require " . var_export($installRoot . '/App/bootstrap.php', true)
        . "; if (Core::VERSION !== '2.0.34' || !isset(TinyCat\\Extension\\Loader::loaded()['release_probe'])) exit(2);";
    [$newBootExit, $newBootOutput] = $run($newBootRunner);
    $assert($newBootExit === 0, 'Updated 2.0.34 runtime and compatible extension boot: ' . implode(' ', $newBootOutput));
